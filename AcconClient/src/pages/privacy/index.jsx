...
export const getServerSideProps = async ({ req }) => {
    ...
    try {
        const getPrivacy = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/Cms/GetPrivacyPage`);
        if (getPrivacy.data.succeeded === false) {
            return {
                redirect: {
                    destination: '/404',
                    permanent: false
            ...
            const getPrivacyPage = getPrivacy.data.data;
            const structuredData = {
                "@context": "https://schema.org",
                "@type": "WebPage",
                "url": siteUrl,
                "name": getPrivacyPage.metaTitle || '',
                "description": getPrivacyPage.metaDescription || '',
                "publisher": {
                    "@type": "Organization",
                    "name": companyName,
                    "url": siteUrl,
            ...
            return {
                props: {
                    ...
                    siteUrl,
                    structuredData,
                    pageInformation: getPrivacyPage,
                }
            };
